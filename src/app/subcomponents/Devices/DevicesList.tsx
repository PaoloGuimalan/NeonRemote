    ...
    <div className='w-full flex flex-col flex-1 bg-transparent overflow-y-scroll x-scroll p-[20px] items-center font-Inter gap-[20px]'>
        <div className="w-full flex flex-row sticky top-0">
            <div className="flex flex-row gap-[20px] items-center">
              ...
              <DialogWidget buttonlabel="Add a device" icon={<IoMdAdd style={{fontSize: "15px", color: "#ffffff"}} />} />
            </div>
        </div>
        {devicelist.length > 0 ? (
          <div className="w-full flex flex-row flex-wrap gap-[10px] justify-center lg:justify-start">
            {devicelist.map((mp: any, i: number) => {
              return(
                <div key={i} onClick={() => { navigate(`/devices/${mp.deviceID.split("_")[1]}`) }} className="flex flex-col border-[2px] rounded-[7px] border-[#e5e6ea] p-[20px] w-full max-w-[250px] h-[170px] max-h-[170px] cursor-pointer select-none">
                  <div className="w-full bg-transparent flex flex-row gap-[10px] items-center">
                    {devicelisticons[mp.type]}
                    <span className="text-[14px] font-semibold flex flex-1">{mp.deviceName}</span>
                  ...
                  <div className="w-full bg-transparent flex flex-row flex-1 gap-[10px] items-end justify-center">
                    <div className={`${mp.isActivated ? "bg-green-500" : "bg-[#b3b3b3]"} p-[5px] pl-[10px] pr-[10px] text-[12px] text-white flex flex-1 justify-center rounded-[3px]`}>
                      {mp.isActivated ? "Activated" : "Not Activated"}
                    </div>
                    <div className={`${mp.isMounted ? "bg-green-500" : "bg-[#b3b3b3]"} p-[5px] pl-[10px] pr-[10px] text-[12px] text-white flex flex-1 justify-center rounded-[3px]`}>
